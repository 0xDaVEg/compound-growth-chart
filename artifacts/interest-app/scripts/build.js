...
function patchHtmlBasePath(dir, basePath) {
  if (!basePath || !fs.existsSync(dir)) return;
  // Negative lookahead: only prefix URLs that don't already start with basePath
  const escapedBase = basePath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const expoRe = new RegExp(`(src|href)="(?!${escapedBase})\\/_expo\\/`, "g");
  const assetsRe = new RegExp(`(src|href)="(?!${escapedBase})\\/assets\\/`, "g");
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const full = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      patchHtmlBasePath(full, basePath);
    } else if (entry.isFile() && entry.name.endsWith(".html")) {
      let html = fs.readFileSync(full, "utf-8");
      const before = html;
      html = html
        .replace(expoRe, `$1="${basePath}/_expo/`)
        .replace(assetsRe, `$1="${basePath}/assets/`);
      fs.writeFileSync(full, html);
      if (html !== before) {
        console.log(`Patched base path in ${entry.name}`);
      } else {
        console.log(`${entry.name} already uses correct base path`);
      }
    }
  }
}
...
async function buildWebExport() {
  ...
  await new Promise((resolve, reject) => {
    ...
    if (proc.stderr) {
      proc.stderr.on("data", (data) => {
        const output = data.toString().trim();
        if (output) console.error(`[expo-web] ${output}`);
      });
    }
  ...
  // Rewrite root-relative asset paths in generated HTML so they work
  // when served from a sub-path (e.g. /mobile/)
  const deployBasePath = (process.env.BASE_PATH || "").replace(/\/+$/, "");
  if (deployBasePath) {
    patchHtmlBasePath(path.join(projectRoot, "dist"), deployBasePath);
  }
}
...
async function main() {
  ...
  if (metroProcess) {
    metroProcess.kill();
    // Give Metro a moment to release its port before web export starts its own
